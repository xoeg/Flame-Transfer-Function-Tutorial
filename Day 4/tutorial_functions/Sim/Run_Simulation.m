function [Mean,Geom,Signals,Sim] = Run_Simulation(u1, T1, p2, Qbar, ...
    ...
    if strcmp(Forcing.Speaker,'ON')
        Geom.Speaker = true;
        % Set up Forcing:
        Mean.Force.Vu = Forcing.Voltage * 1e-2;
        Mean.Force.omega_u = Forcing.Frequency * 2*pi;
    else
        Geom.Speaker = false;
        % Set up Forcing:
        Mean.Force.Vu = 0;
        Mean.Force.omega_u = 0;
    end
    if strcmp(Forcing.Speaker_Downstream,'ON')
        Geom.Speaker_Downstream = true;
        % Set up Forcing:
        Mean.Force.Vd = Forcing.Voltage * 100;
        Mean.Force.omega_d = Forcing.Frequency * 2*pi;
    else
        Geom.Speaker_Downstream = false;
        % Set up Forcing:
        Mean.Force.Vd = 0;
        Mean.Force.omega_d = 0;
    end
